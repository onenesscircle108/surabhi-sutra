...
function getProducts() {
  try {
    ...
    return jsonResponse({
      success: false,
      message: 'getProducts error: ' + err.message
    });
  }
}
...
function saveProduct(data) {
  ...
  try {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      ...
      if (rawId === productId || displayId === productId) {
        sheet.deleteRow(rows[i].sheetRow);
      }
    }
    ...
    return jsonResponse({
      success: false,
      message: 'saveProduct error: ' + err.message
    });
...
function deleteProduct(data) {
  ...
  try {
    ...
    return jsonResponse({
      success: false,
      message: 'deleteProduct error: ' + err.message
    });
...
function saveOrder(data) {
  try {
    ...
    return jsonResponse({
      success: false,
      message: 'saveOrder error: ' + err.message
    });
  }
}
...
function getOrders() {
  try {
    ...
    return jsonResponse({
      success: false,
      message: 'getOrders error: ' + err.message
    });
  }
}
...
function getReviews(e) {
  try {
    ...
    if (!sheet) {
      return jsonResponse({
        success: false,
        message: 'Reviews sheet not found'
      });
    }
    ...
    return jsonResponse({
      success: false,
      message: 'getReviews error: ' + err.message
    });
  }
}
...
function saveReview(data) {
  try {
    ...
    return jsonResponse({
      success: false,
      message: 'saveReview error: ' + err.message
    });
  }
}
...
/* ======================================================
   ADMIN AUTH
   Sheet: Admin_id
   Columns: Admin_id (A) | Name (B) | Email (C) | Password (D)

   HOW TO SET UP:
   1. In your Google Sheet, create a tab named exactly: Admin_id
   2. Row 1 = headers: Admin_id | Name | Email | Password
   3. Row 2+ = one admin per row, e.g.:
        ADM-001 | Sarthak | sarthak@example.com | mypassword
   4. Redeploy as a new version after pasting this code.

   LOGIN FLOW (2-step):
   Step 1 — verifyAdmin:    checks email + password → sends OTP to admin email
   Step 2 — verifyAdminOTP: checks OTP → returns admin data → unlocks dashboard
====================================================== */
...
function verifyAdmin(data) {
  try {
    const sheet = getSheet(ADMIN_SHEET);
    if (!sheet) return jsonResponse({ success: false, message: 'Admin_id sheet not found. Please set it up first.' });
    ...
    // Generate 6-digit OTP and store server-side (keyed per email)
...
/* ── Step 2: verify OTP → unlock dashboard ── */
function verifyAdminOTP(data) {
  try {
    ...
    if (!storedOTP) {
      return jsonResponse({ success: false, message: 'No OTP found. Please sign in again.' });
    }
    ...
    // Return admin data to populate the dashboard
    const sheet = getSheet(ADMIN_SHEET);
...
function deleteAdmin(data) {
  ...
  try {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      if (String(rows[i].values[0] || '').trim() === adminId) {
        sheet.deleteRow(rows[i].sheetRow);
      }
    }
...
/* ======================================================
   VENDORS
   Stored in Script Properties as JSON object:
   { "VND-001": { id, name, created }, ... }
====================================================== */
...
function saveVendor(data) {
  ...
  try {
    ...
    // If this name already exists (case-insensitive), return existing record
